...
export default function About() {
  ...
  return (
    <section id="about" className="section-shell scroll-mt-24 relative bg-[#0B0F14] overflow-hidden">
      <div className="absolute top-1/2 left-0 -translate-y-1/2 w-[320px] h-[320px] bg-[#2EE6D6]/5 rounded-full blur-[100px] pointer-events-none" />
              ...
              <div className="surface-card p-5 md:p-6 rounded-2xl">
                ...
                <div className="flex flex-wrap gap-2 mt-5 pt-5 border-t border-white/5">
                  <a href={personalInfo.linkedin} target="_blank" rel="noopener noreferrer"
                    ...
                    <FiLinkedin className="text-[#2EE6D6]" /> LinkedIn <FiExternalLink size={12} />
                  </a>
                  <a href={personalInfo.github} target="_blank" rel="noopener noreferrer"
                     className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-[0.7rem] font-semibold text-slate-400 hover:text-white transition-all">
                    <FiGithub /> GitHub <FiExternalLink size={12} />
                  </a>
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#2EE6D6]/10 border border-[#2EE6D6]/20 text-[0.7rem] font-semibold text-[#2EE6D6]">
            ...
            <div className="lg:col-span-5 flex flex-col gap-3">
              {traits.map((trait) => (
                ...
                >
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-white mb-1 tracking-tight group-hover:text-[#2EE6D6] transition-colors">{trait.title}</h3>
                    <p className="text-slate-500 text-xs leading-relaxed">{trait.desc}</p>
                  </div>
                </motion.div>
              ))}

              <motion.div variants={itemVariants} className="mt-1 p-4 rounded-2xl bg-[#2EE6D6]/8 border border-[#2EE6D6]/20 flex justify-around items-center">
                 <div className="text-center">
                    <span className="block text-lg font-bold text-white tracking-tight">3+</span>
                    <span className="text-[0.55rem] font-semibold uppercase tracking-wider text-[#2EE6D6]/80">Years</span>
                 </div>
                 <div className="w-px h-8 bg-white/10" />
                 <div className="text-center">
                    <span className="block text-lg font-bold text-white tracking-tight">10+</span>
                    <span className="text-[0.55rem] font-semibold uppercase tracking-wider text-[#2EE6D6]/80">Shipped</span>
                 </div>
                 <div className="w-px h-8 bg-white/10" />
                 <div className="text-center">
                    <span className="block text-lg font-bold text-white tracking-tight">M.Sc.</span>
                    <span className="text-[0.55rem] font-semibold uppercase tracking-wider text-[#2EE6D6]/80">CS</span>
                 </div>
              </motion.div>
            </div>
          </div>
